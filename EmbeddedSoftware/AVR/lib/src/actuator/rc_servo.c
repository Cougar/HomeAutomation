...
#if NUMBER_OF_RCS > 1
        case 2:
            temp_pos = (uint8_t)PWM2_CENTER_PULSE + (int8_t)((abs_pos*STEP2)/8);
            if(temp_pos<PWM2_MIN_PULSE){
                OCR0A = PWM2_MIN_PULSE;
            }else if(temp_pos>PWM2_MAX_PULSE){
...
#if NUMBER_OF_RCS > 2
        case 3:
            temp_pos = (uint8_t)PWM3_CENTER_PULSE + (int8_t)((abs_pos*STEP3)/8);
            if(temp_pos<PWM3_MIN_PULSE){
                OCR0B = PWM3_MIN_PULSE;
            }else if(temp_pos>PWM3_MAX_PULSE){
