...
static uint8_t Can_MessagesAvailableInController(void) __attribute__((unused));
static Can_Return_t Can_ControllerCheckError(void) __attribute__((unused));
...
void Can_Service() {
		...
		for (uint8_t i=0; i<CAN_CONTROLLER_NR_TX_BUFFERS; i++) {
			if (canTxQueue.nrElements > 0) {
				...
				//TODO: dont enable interrupts, restore previous state
				sei();
			}
			else {
...
Can_Return_t Can_Receive(Can_Message_t *msg) {
		...
		for (i=0; i<CAN_CONTROLLER_NR_RX_BUFFERS; i++) {
 ...
 * 
 * @todo
 * 		Styr upp denna funktion lite b�ttre.
 */
