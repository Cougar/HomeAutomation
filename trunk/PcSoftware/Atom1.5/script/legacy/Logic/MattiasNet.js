...
SensorPrint.prototype.myIrReceiverService = null;
SensorPrint.prototype.myDimmer230ServiceA = null;
SensorPrint.prototype.myDimmer230ServiceB = null;
...
SensorPrint.prototype.initialize = function(initialArguments)
{
	...
	if (!this.myInitialArguments["irReceive"])
	{
		log(this.myName + ":" + this.myId + "> Failed to initialize, irReceive-config missing from config.\n");
		return;
	}

	if (!this.myInitialArguments["Dimmer230A"])
	{
		log(this.myName + ":" + this.myId + "> Failed to initialize, Dimmer230A-config missing from config.\n");
		return;
	}
	
	if (!this.myInitialArguments["Dimmer230B"])
	{
		log(this.myName + ":" + this.myId + "> Failed to initialize, Dimmer230B-config missing from config.\n");
		return;
	}
	...
	/* Get the LCD service that we want from the ServiceManager, it takes type, service name, service id */
	this.myDimmer230ServiceA = ServiceManager.getService("Can", "Dimmer230", this.myInitialArguments["Dimmer230A"]["Id"]);
	/* Add a callback for when the sensor reports a new value */
	this.myDimmer230ServiceA.registerEventCallback("online", function(args) { self.dimmerAOnline(); });
	/* If the service is already online we should call the handler here */
	this.dimmerAOnline();
	...
	/* Get the LCD service that we want from the ServiceManager, it takes type, service name, service id */
	this.myDimmer230ServiceB = ServiceManager.getService("Can", "Dimmer230", this.myInitialArguments["Dimmer230B"]["Id"]);
	/* Add a callback for when the sensor reports a new value */
	this.myDimmer230ServiceB.registerEventCallback("online", function(args) { self.dimmerBOnline(); });
	/* If the service is already online we should call the handler here */
	this.dimmerBOnline();
	...
	/* Get the LCD service that we want from the ServiceManager, it takes type, service name, service id */
	this.myIrReceiveService = ServiceManager.getService("Can", "irReceive", this.myInitialArguments["irReceive"]["Id"]);
	/* Add a callback for when the sensor reports a new value */
	this.myIrReceiveService.registerEventCallback("newIRdata", function(args) { self.ireceiveUpdate(args); });
	/* Add a callback for when the service goes online */
	this.myVoltService.registerEventCallback("online", function(args) { self.irreceiveOnline(); });
	/* If the service is already online we should call the handler here */
	this.irreceiveOnline();
...
SensorPrint.prototype.dimmer230AOnline = function()
{
}

SensorPrint.prototype.dimmer230BOnline = function()
{
}
