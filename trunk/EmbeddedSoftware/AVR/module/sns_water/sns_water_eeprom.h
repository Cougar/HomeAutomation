#ifndef SNS_WATER_EEPROM
#define SNS_WATER_EEPROM
...
// ----------------------------------------------------------------
	...
	#define EEDATA 			(uint8_t*)&eeprom_sns_water.Data
	#define EEDATA16		(uint16_t*)&eeprom_sns_water.Data
	#define EEDATA32		(uint32_t*)&eeprom_sns_water.Data
	#define EEDATA_CALC_CRC		eeprom_crc8((uint8_t*)&eeprom_sns_water.Data,sizeof(struct sns_water_Data))
	...
	#define EEDATA_UPDATE_CRC	eeprom_update_byte((uint8_t*)&eeprom_sns_water.crc,eeprom_crc8((uint8_t*)&eeprom_sns_water.Data,sizeof(struct sns_water_Data)))
#else
	#define EEDATA_UPDATE_CRC	eeprom_write_byte((uint8_t*)&eeprom_sns_water.crc,eeprom_crc8((uint8_t*)&eeprom_sns_water.Data,sizeof(struct sns_water_Data)))
#endif

	#define EEDATA_OK		EEDATA_CALC_CRC == EEDATA_STORED_CRC
	#define EEDATA_STORED_CRC 	eeprom_read_byte((uint8_t*)&eeprom_sns_water.crc)
	
	struct eeprom_sns_water{
		struct sns_water_Data Data;
		uint8_t crc;
	};
	extern struct eeprom_sns_water EEMEM eeprom_sns_water;
