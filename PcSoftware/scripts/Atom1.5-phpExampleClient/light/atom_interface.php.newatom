...
function atomd_read_packet($socket)
{
  ...
  if ($payload_length > 0)
  {
    $payload = fread($socket, intval($payload_length)-1);
  ...
  return $command . str_pad(strlen($payload)-1, 4, "0", STR_PAD_LEFT) . $payload;
}
...
function atomd_write_packet($socket, $command, $payload)
{
  ...
  $packet = $command . str_pad(strlen($payload), 4, "0", STR_PAD_LEFT) . $payload;
