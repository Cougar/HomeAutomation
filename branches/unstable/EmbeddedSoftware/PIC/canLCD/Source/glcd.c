/*********************************************************************
 *
 *                  Graphical LCD routines (LM6038D (ST7565))
 *
 *********************************************************************
 * FileName:        $HeadURL: http://svn.arune.se/svn/HomeAutomation/trunk/EmbeddedSoftware/PIC/canLCD/Source/lcd.c $
...
static void glcdCommand(BYTE);
static void glcdDataWrite(BYTE);
static void glcdSetPage(BYTE);
static void glcdSetColumn(BYTE);
...
void glcdInit(void)
{
	...
	// SPI (one way)
	// /SMP, /CKE, na, na, na, na, na, na
	SSPSTAT = 0b00000000;
	// na, na, SSPEN, CKP, SSPM3:0(FOSC/4)
	SSPCON1 = 0b00110001;

	GLCD_RES_IO = 0;
	Delay100TCYx(1);	// 10 us (2.5uS needed)
	GLCD_RES_IO = 1;

	glcdCommand(0xA2); // LCD bias = 1/9
	glcdCommand(0xA0); // ADC = normal
	glcdCommand(0xC8); // SHL = reverse
	glcdCommand(0x40); // Init display line = 0
	glcdCommand(0x2C); // PWR VF=0,VR=0,VC=1
	Delay10KTCYx(50);  // delay 50ms
	glcdCommand(0x2E); // PWR VF=0,VR=1,VC=1
	Delay10KTCYx(50);  // delay 50ms
	glcdCommand(0x2F); // PWR VF=1,VR=1,VC=1
	Delay10KTCYx(50);  // delay 50ms
	glcdCommand(0x25); // Regulator resistor select
	glcdCommand(0x81); // Set reference voltage mode
	glcdCommand(0x20); // set reference voltage resistor
	glcdCommand(0xAF); // Display ON


	/*
	glcdCommand(0xA2); // LCD bias set
	glcdCommand(0xA1); // ADC select
	glcdCommand(0xC0); // Common select
	glcdCommand(0xA6); // Display set
	glcdCommand(0xA4); // point ON/OFF
	glcdCommand(0x81); // ???
	glcdCommand(0x1C); // ???
	glcdCommand(0x2F); // Power set
	glcdCommand(0xF8); // Booster set
	glcdCommand(0x01); // ??
	glcdCommand(0xAF); // Display ON
	*/

	Delay100TCYx(5); // delay 50us
}


void glcdCommand(BYTE command)
{
	GLCD_A0_IO = 0;
	PIR1bits.SSPIF=0;
	SSPBUF = command;
...
void glcdSetStartLine(BYTE startline)
{
	glcdCommand((startline & 0x3F) | 0x40);
}

void glcdSetPage(BYTE page)
{
	glcdCommand((page & 0x0F) | 0xB0);
}

void glcdSetColumn(BYTE column)
{
	glcdCommand((column & 0x0F));
	glcdCommand(((column >> 4) & 0x0F) | 0x10);
}
...
void glcdDisplayPic(BYTE page, BYTE column, const rom BYTE * pic)
{
	BYTE i,j;
	for(i=0;i<8;i++)
	{
		glcdSetPage(page+i);
		glcdSetColumn(column);
		for(j=0;j<128;j++)
		{
			glcdDataWrite(pic[i*128+j]);
		}
	}
}

void glcdPutc(BYTE c)
{
	BYTE i;
	for(i=0;i<FONT_WIDTH;i++)
	{
		glcdDataWrite(Font8x6[c][i]);
	}	
}

void glcdPutrs(BYTE page, BYTE column, const rom BYTE * s)
{
	glcdSetPage(page);
	glcdSetColumn(column);
	while(*s) glcdPutc(*s++);
...
void glcdPuts(BYTE page, BYTE column, BYTE * s)
{
	glcdSetPage(page);
	glcdSetColumn(column);
	while(*s) glcdPutc(*s++);
