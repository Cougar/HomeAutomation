...
#define IR_COMP_VECTOR	TIMER1_COMPB_vect
#define IR_OVF_VECTOR	TIMER1_COMPA_vect
#define IR_CAPT_VECTOR	TIMER1_CAPT_vect
#define IR_TIMEOUT_REG	OCR1A
#define IR_PW_REG		OCR1B
#define IR_CAPT_REG		ICR1
#define IR_TIMER_INIT()	TCCR1A = 0; \
						TCCR1B = (1<<ICNC1)|(1<<WGM12)|(2<<CS10);
#define IR_MASK_COMP()	TIMSK1 &= ~(1<<OCIE1B);
#define IR_UNMASK_COMP()	TIFR1 = (1<<OCF1B); TIMSK1 |= (1<<OCIE1B);
#define IR_MASK_OVF()	TIMSK1 &= ~(1<<OCIE1A);
#define IR_UNMASK_OVF()	TIFR1 = (1<<OCF1A); TIMSK1 |= (1<<OCIE1A);
#define IR_MASK_CAPT()	TIMSK1 &= ~(1<<ICIE1);
#define IR_UNMASK_CAPT()	TIFR1 = (1<<ICF1); TIMSK1 |= (1<<ICIE1);
#define IR_CAPT_FALLING()	TCCR1B &= ~(1<<ICES1); \
							TIFR1 = (1<<ICF1);
#define IR_CAPT_RISING()	TCCR1B |= (1<<ICES1); \
							TIFR1 = (1<<ICF1);
...
ISR(IR_CAPT_VECTOR)
{
	/* Read the measured pulse time from the capture register. */
	uint16_t pulsewidth = IR_CAPT_REG;
	
	/* Subtract the current measurement from the counter instead of setting it
	 * to zero so that the following measurement is not affected by variations
	 * in the interrupt response time. */ 
	TCNT1 -= pulsewidth;
	//TCNT1 = 0;
